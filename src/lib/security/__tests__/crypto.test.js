import { savePin, getEncryptionKey, clear } from '../crypto'
...
describe('Crypto', () => {
  ...
  test('should throw error when pin doesn\'t set', async () => {
    let key
    try {
  ...
  test('should generate encryption key when valid pin is set', async () => {
    const pin = '123456'
    await savePin(pin)

    const encryption = await getEncryptionKey()
  ...
  test('should throw error when pin set is null', async () => {
    const pin = null
